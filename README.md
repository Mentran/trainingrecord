...
- 启动页面时会比较浏览器与 `data/app-data.json` 的更新时间，保留较新版本；本地文件服务 3 秒未响应时自动使用浏览器数据。
- 文件读写均使用共享 Schema 校验，POST 请求体上限为 5MB；非法或超大请求不会覆盖现有文件。
...
- JSON 导入会做实体级校验，支持覆盖或按 ID 合并；导入失败自动回滚，成功后可撤销上次导入。
...
| 2026-07-06 | v1.1 | 新增训练提示 MVP：网球等级设置、首页今日训练提示、20 条本地卡片、收藏到技巧笔记和设为今日关注 |
| 2026-07-10 | v1.2 | 完成数据安全基线：Schema 校验、原子导入与撤销、多运动隔离、文件冲突策略、自动化测试与 CI |
